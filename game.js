(() => {
  ...
  const CFG = {
    gravity: 826, diveGravityMultiplier: 4.2, glideLift: 255, glideSpeedLiftFactor: .33, glideSpeedLiftMax: 490, flapLift: 75,
    groundFriction: .085, airDrag: .022, maxSpeed: 1800,
  ...
  function drawGround(){const left=cameraX-8,right=cameraX+W/cameraZoom+8,bottom=cameraY+H/cameraZoom;ctx.beginPath();ctx.moveTo(left,bottom);for(let x=left;x<=right;x+=7)ctx.lineTo(x,terrainAt(x).y);ctx.lineTo(right,bottom);ctx.closePath();ctx.fillStyle='#5d9a79';ctx.fill();ctx.beginPath();for(let x=left;x<=right;x+=7)ctx.lineTo(x,terrainAt(x).y);ctx.strokeStyle='#d7eeaa';ctx.lineWidth=7;ctx.stroke();ctx.beginPath();for(let x=left;x<=right;x+=7)ctx.lineTo(x,terrainAt(x).y-2);ctx.strokeStyle='#77b582';ctx.lineWidth=2;ctx.stroke()}
  function screenBody(){return{x:(body.position.x-cameraX)*cameraZoom,y:(body.position.y-cameraY)*cameraZoom}}
  function drawBird(){const p=body.position, speed=mag(body.velocity), gliding=body.state==='Airborne'&&!held;const ang=body.state==='Grounded'?terrainAt(body.position.x).slopeAngle:Math.atan2(body.velocity.y,body.velocity.x)*.68;ctx.save();ctx.translate(p.x,p.y);ctx.rotate(ang);if(held&&body.state==='Grounded'){ctx.globalAlpha=.18;ctx.fillStyle='#fff4a8';ctx.beginPath();ctx.arc(0,5,25+Math.min(20,speed*.025),0,7);ctx.fill();ctx.globalAlpha=1}const wingAngle=gliding?-.45+Math.sin(flapTime*15)*.72:.42;ctx.save();ctx.rotate(wingAngle);ctx.fillStyle='#df6f6b';ctx.beginPath();ctx.ellipse(-8,0,16,7,0,0,7);ctx.fill();ctx.fillStyle='#f59b83';ctx.beginPath();ctx.ellipse(-10,-1,8,3.5,0,0,7);ctx.fill();ctx.restore();ctx.fillStyle='#f28c73';ctx.beginPath();ctx.ellipse(0,0,19,16,0,0,7);ctx.fill();ctx.fillStyle='#f7b28e';ctx.beginPath();ctx.ellipse(4,7,11,6,0,0,7);ctx.fill();ctx.fillStyle='#f3c760';ctx.beginPath();ctx.moveTo(16,-1);ctx.lineTo(27,4);ctx.lineTo(16,7);ctx.closePath();ctx.fill();ctx.fillStyle='#344d5b';ctx.beginPath();ctx.arc(8,-6,2.7,0,7);ctx.fill();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(9,-7,1,0,7);ctx.fill();ctx.restore()}
  function spawnDust(){const p=body.position;for(let i=0;i<3;i++)particles.push({x:p.x-10,y:p.y+12,vx:-20-Math.random()*70,vy:-15-Math.random()*35,life:.5+Math.random()*.25,c:'#f8eab3'})}
  ...
  function drawRings(){for(const r of rings){if(r.x<cameraX-30||r.x>cameraX+W/cameraZoom+30||r.got)continue;ctx.save();ctx.translate(r.x,r.y);ctx.rotate(Math.sin((cameraX+r.x)*.008)*.14);ctx.strokeStyle='#fff0a6';ctx.lineWidth=6;ctx.shadowColor='#fff8b6';ctx.shadowBlur=11;ctx.beginPath();ctx.arc(0,0,17,0,7);ctx.stroke();ctx.restore()}}
  ...
  function updateGround(dt,surface){
    ...
    const posture=held?lerp(CFG.groundDiveMultiplier,CFG.diveMinimumMultiplier,clamp(v/CFG.diveSoftCap,0,1)):CFG.groundGlideMultiplier;
